...
import { Download, FileDown, Calendar, Printer } from "lucide-react";
...
export default function Reports() {
  ...
  // LOAD DATA EVERY TIME USER ENTERS PAGE
  useEffect(() => {
    loadData();
  }, []);
  ...
  // FIXED: safer date filtering
  const getDate = (d: string) => new Date(d.replace(" ", "T"));
  ...
  const getFilteredBills = () => {
    ...
    switch (selectedPeriod) {
      case "today":
        return bills.filter((b) => getDate(b.createdAt) >= today);

      case "week":
        const weekAgo = new Date(today);
        weekAgo.setDate(weekAgo.getDate() - 7);
        return bills.filter((b) => getDate(b.createdAt) >= weekAgo);

      case "month":
        const monthAgo = new Date(today);
        monthAgo.setMonth(monthAgo.getMonth() - 1);
        return bills.filter((b) => getDate(b.createdAt) >= monthAgo);
